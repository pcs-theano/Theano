...
from theano.tensor.nnet import conv_mkldnn, conv, conversionOp
...
class TestCorr2D(utt.InferShapeTester):
    ...
    def validate(self, image_shape, filter_shape,
                 ...
                 verify_grad=True, non_contiguous=False, uniq_name=None):
        """
        :param image_shape: The constant shape info passed to conv_forward.
        :param filter_shape: The constant shape info passed to conv_forward.
        """
        ...
        if border_mode == 'valid':
            ...
            padding = (filter_shape[2] - 1, filter_shape[3] - 1)
        elif border_mode == 'half':
            padding = (filter_shape[2]/2, filter_shape[3]/2)
        elif isinstance(border_mode, tuple):
            padding = border_mode
        else:
            padding = (border_mode, border_mode)

        # we create a symbolic function so that verify_grad can work
        def sym_Conv(input, filters, uniq_name=None):
            # define theano graph and function
            input.name = 'input'
            filters.name = 'filters'
            u2iOp = conversionOp.U2I('conv','dnnResourceSrc', tuple(filter_shape)+tuple(image_shape)+tuple(subsample)+(0,0)+(1,), uniq_name=uniq_name)
            u2i_out = u2iOp(input)
            rval = conv_mkldnn.conv_forward(border_mode, subsample, imshp=image_shape, kshp=filter_shape)(u2i_out, filters)
            i2uOp = conversionOp.I2U(uniq_name=uniq_name)
            i2u_out = i2uOp(rval)
            i2u_out.name = 'conv forward'
            return i2u_out
            #rval.name = 'conv forward'
            #return rval

        output = sym_Conv(input, filters)
        output.name = 'conv_forward()(%s,%s)' % (input.name, filters.name)
        ...
        #print ("mkldnn_conv_result:")
        #print (theano_output)
        #print ('*' * 30)
        #print ("reference_conv_result:")
        #print (ref_output)
        ...
        if verify_grad:
            print ("verify grad")
            utt.verify_grad(sym_Conv, [orig_image_data, filter_data],
                            mode=self.mode)
    ...
    def test_basic(self):
        ...
        #              (4, 2, 5, 5), (5, 2, 2, 3)]
        border_modes = ['valid']
        img_shapes = [(3, 2, 8, 8), (3, 2, 4, 4), (3, 2, 7, 5),
                      (3, 2, 8, 9), (3, 2, 7, 5)]
        fil_shapes = [(4, 2, 5, 5), (4, 2, 2, 2), (5, 2, 3, 2),
                      (4, 2, 5, 4), (5, 2, 2, 3)]
        ...
        #        count+=1
        #        print ("count=%d"%count)
        #        self.validate(img, fil, border_mode, verify_grad=False, uniq_name=str(count))
    ...
    def test_img_kernel_same_shape(self):
        ...
        #self.validate((3, 2, 3, 3), (4, 2, 3, 3), 'half') # failed in grad process, filter and padding are minus????!!!
    ...
    def test_shape_Constant_tensor(self):
        ...
        border_modes = ['valid', (1, 1), (2, 2), (3, 3), 1]
        ...
        for aivec_val, bivec_val in zip(aivec_vals, bivec_vals):
            ...
            for mode in modes:
                for subsample in subsamples:
                    # CorrMM
                    cdtens = conv(border_mode=mode, subsample=subsample, imshp=tuple(aivec_val), kshp=tuple(bivec_val))(adtens, bdtens)
                    self._compile_and_check([adtens, bdtens],
                                            [cdtens],
                                            [adtens_val, bdtens_val], conv,
                                            warn=False)
    ...
    def test_infer_shape_gradW(self):
        ...
        def rand(*shape):
            ...
            return r * 2 - 1
        conv = conv_mkldnn.conv_forward
        gradW = conv_mkldnn.conv_gradWeights
            ...
            for mode in modes:
                for subsample in subsamples:
                    ...
                    bdtens_g = gradW(border_mode=mode,
                                     subsample=subsample, imshp=tuple(aivec_val), kshp=tuple(bivec_val))(adtens, cdtens, shape=shape)
    ...
    def test_infer_shape_gradI(self):
        ...
        def rand(*shape):
            ...
            return r * 2 - 1
        conv = conv_mkldnn.conv_forward
        gradI = conv_mkldnn.conv_gradInputs
            ...
            for mode in modes:
                for subsample in subsamples:
                    ...
                    adtens_g = gradI(border_mode=mode,
                                     subsample=subsample, imshp=tuple(aivec_val), kshp=tuple(bivec_val))(bdtens, cdtens, shape=shape)
    ...
    def test_non_contiguous(self):
        ...
        #self.validate((3, 2, 8, 8), (4, 2, 5, 5), 'half', non_contiguous=True)  # failed in grad process, filter and padding are minus????!!!
        ...
        self.validate((3, 2, 7, 5), (5, 2, 2, 3), (2, 2), non_contiguous=True)
