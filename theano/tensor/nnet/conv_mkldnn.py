"""
contains an op for convolving input images with a set of weights by using mkldnn
...
class MKLDNNBase(gof.Op):
    __props__ = ('border_mode', 'subsample', 'imshp', 'kshp', 'uniq_id')

    def __init__(self, border_mode="valid", subsample=(1, 1), imshp=None, kshp=None, uniq_id=0):
        if (not theano.config.blas.ldflags) or ('mkl' not in theano.config.blas.ldflags):
            raise NotImplementedError("Convolution with MKLDNN requires MKL library.")
    ...
    def c_compile_args(self):
        ...
        compile_args += super(MKLDNNBase, self).c_compile_args()
        return compile_args
    ...
    def c_headers(self):
        headers = ['<stdio.h>']
        headers += super(MKLDNNBase, self).c_headers()
        return headers
...
class conv_forward(MKLDNNBase):
    def c_cleanup_code_struct(self, node, name):
        ...
        ccode = """
            ...
            //dnnLayoutDelete_%(precision)s(bias_int_layout);
            //dnnLayoutDelete_%(precision)s(top_int_layout);
            //END
        """ % locals()
        return ccode

    def make_node(self, img, kern, bias):
        ...
        return Apply(self, [img, kern, bias], [TensorType(dtype, broadcastable)()])
    ...
    def c_code(self, node, name, inp, out_, sub):
        ...
        ccode = """
            ...
            if (1 == first_run) {
                ...
                if (!dnnLayoutCompare_%(precision)s(bias_usr_layout, bias_int_layout)) {
            ...
            conv_res[dnnResourceDst] = top_buffer_ptr;
                ...
                std::cout << "forward, conv_res[Bias] = @" << conv_res[dnnResourceBias] << std::endl;
            #endif
    ...
    def grad(self, inp, grads):
        ...
        top, = grads
        d_image = conv_gradInputs(self.border_mode,
                                    self.subsample,
                                    imshp=self.imshp,
                                    kshp=self.kshp)(bottom, weights, top)
        d_weights, d_bias = conv_gradWeights(self.border_mode,
                                    self.subsample,
                                    imshp=self.imshp,
                                    kshp=self.kshp)(bottom, weights, top, bias)
        return d_image, d_weights, d_bias


class conv_gradWeights(MKLDNNBase):
    #def infer_shape(self, node, input_shape):
    ...
    def make_node(self, image, weight, topgrad, bias):
        ...
        return Apply(self, [image, weight, topgrad, bias], [weight.type(), bias.type()])
    ...
    def c_code(self, node, name, inp, out_, sub):
        ...
        ccode = """
            #if __DEBUG__
                std::cout << "gradWeight, c_code start" << std::endl;
            #endif
            //bwdfilter related
            ...
            if ( !(%(weightgrad)s) ) {
                   //&& PyArray_NDIM(%(weightgrad)s) == 5
                   //&& PyArray_IS_C_CONTIGUOUS(%(weightgrad)s))){
            ...
            if ( !(%(biasgrad)s) ) {
                ...
                if (NULL == %(biasgrad)s) {
                    PyErr_Format(PyExc_RuntimeError,
                            "conv_backward: Failed to allocate bias of %%lld",
...
class conv_gradInputs(MKLDNNBase):
    #def infer_shape(self, node, input_shape):
    #    if self.border_mode == "half":
    #        padH = padW = -1
    #    elif self.border_mode == "full":
