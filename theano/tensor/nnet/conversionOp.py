...
class U2I(gof.Op):
    ...
    def __hash__(self):
       return hash(type(self)) ^ hash(self.params) ^ hash(self.Op) ^ hash(self.target) ^ hash(self.uniq_name)
    ...
    def grad(self, inp, grads):
        x, = inp
        gz, = grads
        return [U2IGrad(self.Op,self.target,self.params, self.uniq_name)(x, gz)]
        #return [I2U()(gz)]
    ...
    def c_init_code_struct(self, node, name, sub):
        ...
        if 'maxPool' == self.Op:
            ...
            ccode += """
             ...
             if(0!=status)
             {
                  printf(\"ERROR:Create Usr layout for max pool\\n\");
                  exit(0);
             }
            ...
            ccode += """
             ...
             size_t stride[2] = {%(dW)s, %(dH)s};
             size_t bottom_size[4] = {%(W)s,%(H)s,%(C)s,%(N)s};
             size_t bottome_stride[4] = {1,%(W)s, %(W)s*%(H)s, %(W)s*%(H)s*%(C)s};
             ...
             int status = dnnGroupsConvolutionCreateForwardBias_%(precision)s(&OpFwd,//Bias
                                         NULL,///attributes,
